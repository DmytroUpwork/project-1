...
//   openDocument('https://www.bestchange.ru/' + cu_list[id2pos(fromcur)] + '-to-' + cu_list[id2pos(tocur)] + '.html', false);
// }
...
//     if (a_id.name) a_id.href = 'https://www.bestchange.ru/' + a_id.name;
//   }
// }
...
//     set_cur_cookies();
//     openDocument('https://www.bestchange.ru/' + direct_data[id]['u'] + '-exchanger.html', false);
//   }
//   return false;
// }
...
//       nodeById('det_changer').innerHTML = '<a target="_blank" href="https://www.bestchange.ru/click.php?' + direct_data[id]['a'] + '" title="Открыть обменник">' + direct_data[id]['n'] + '</a>' + (direct_data[id]['v'] ? '<span class="full" title="Данный обменный пункт прошел дополнительную проверку"></span>' : '') + (direct_data[id]['o'] ? '<span class="company" title="Данный обменный пункт имеет официальную юридическую регистрацию"></span>' : '') + (star_content ? '<span class="stars" title="Рейтинг обменника">' + star_content + '</span>' : '');
//       nodeById('det_bl').innerHTML = (is_wmid ? '<a target="_blank" rel="noreferrer" href="https://www.bestchange.ru/click.php?url=' + encodeURIComponent('https://passport.webmoney.ru/asp/CertView.asp?wmid=') + direct_data[id]['w'] + '&noref=1" title="Посмотреть WMID ' + direct_data[id]['w'] + '">' + direct_data[id]['b'] + '</a>' : '&mdash;');
...
//   openDocument('tg://resolve?domain=bestchange_bot&start=n-1-' + id, true);
//   start_tglink_checking();
// }
...
//         set_status(email_type, 'Необходимо дать согласие на обработку данных', 'error.png');
...
//   if (bonus_captcha_obj) bonus_captcha = hcaptcha.render(bonus_captcha_obj, {
...
//   });
//   setTimeout(function() {
//     removeClass(bonus_captcha_obj, 'hide');
//   }, 1000);
...
//         if (results.length == 6) {
//           if (results[0] == 'OK') {
//             set_bonus_status(results[5] + '<p>Если вам понравился наш мониторинг обменников, вы можете:</p><ul><li>Написать свой отзыв на <a target="_blank" rel="noreferrer" href="https://www.mywot.com/ru/scorecard/bestchange.com">MyWOT</a> или во <a target="_blank" href="https://vk.com/topic-53507345_39091228">ВКонтакте</a> (мы очень любим получать отзывы);</li><li><a href="/partner/">Зарегистрироваться</a> в нашей партнерской программе и зарабатывать деньги, <span class="videolink" onclick="show_video(\'aff-ru\')"><span class="videoicon"></span><span class="link dashlink">рекламируя</span></span> BestChange;</li><li>Посоветовать BestChange вашим друзьям и знакомым;</li><li>Подписаться на нашу группу ВКонтакте:</li></ul><iframe src="/js/vk.html" align="middle" width="266" height="250" allowTransparency="allowTransparency" scrolling="no" border="0" frameborder="0" seamless="seamless"></iframe><input type="button" class="button" value="Спасибо! Я хочу еще!" onclick="open_bonus_form()" />', results[1], 'ok.png');
...
//   if (!confirm('Вы уверены, что хотите получить выплату на кошелек?')) return false;
...
//       faucet_order_button.value = 'Заказать выплату';
//     }, 25000);
//     faucet_order_button.disabled = true;
//     faucet_order_button.value = 'Заказ выплаты...';
...
//           document.getElementById('faucet_unpaid_balance').innerHTML = '0 сатоши';
//           show_th_info = false;
//           show_trans_history();
//         } else {
//           faucet_order_button.disabled = false;
...
//   openDocument('https://www.bestchange.ru/' + cu_list[id2pos(from)] + '-to-' + cu_list[id2pos(to)] + '.html', false);
// }
...
//           }, 1000);
//         } else {
//           set_status('fink', results[1] ? results[1] : 'Информация не отправлена', 'error.png');
//         }
//       } else {
...
// function prepare_ol(ol, current_ol_name, rule) {
...
//     a.name = new_ol_name;
//     lis[i].prepend(a);
//     if (new_ol_name == rule) lis[i].style.backgroundColor = '#ffffaa';
//     var ols = lis[i].getElementsByTagName('ol');
//     if (ols.length > 0) prepare_ol(ols[0], new_ol_name, rule);
//   }
// }
...
//   }
//   for (i = 0; i < main_ol_list.length; i++) prepare_ol(main_ol_list[i], main_ol_list.length > 1 ? i + 1 : '', rule);
...
//       if (as[i].name == rule) {
//         as[i].parentNode.style.backgroundColor = '#ffffaa';
//       } else {
